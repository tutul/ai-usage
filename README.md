...
<p>
  <img src="docs/images/menu.png" width="280" alt="menu bar 點開：兩家本週用量、重置倒數、追蹤開關">
  <img src="docs/images/usage.png" width="480" alt="用量歷史：日粒度折線圖，斷開處代表沒有取樣">
</p>

點開 menu bar 圖示看本週用量與重置倒數；「歷史圖表」可按小時／日／週回溯，
沒有取樣的區間會斷開、不補值。

<img src="docs/images/cache.png" width="480" alt="快取分頁：涵蓋率、讀寫量，按日與專案分列">

「快取」分頁從本機對話紀錄匯入，按日、按專案列出快取的讀寫與閒置後重寫。

## 這是給誰用的

**Claude 桌面版**與 **Codex** 的訂閱使用者（Claude Pro／Max、ChatGPT 訂閱）。

**不適合：**
- **常用 `claude` CLI 的人** —— 本 app 續期憑證後，CLI 手上那份就作廢，會一再被登出
  （見[憑證怎麼來](#憑證怎麼來)）。
- **用 API key 按量計費的人** —— 沒有週額度可看。

兩家都沒有公開的訂閱用量 API，本 app 是借用既有的登入狀態，因此：

- **Claude 仍需裝 Claude Code 並登入一次**，只為了取得初始憑證 —— 桌面版的登入存在它自己的 cookie 裡，讀不到。
- **只有百分比與重置時間**，沒有 token 數或金額；百分比是整個帳號的額度，其他裝置的用量也算在內。
- **快取分頁只含這台 Mac 的對話紀錄**，對不上用量百分比是正常的。
- **只支援 macOS。**
...
|---|---|
| 藍色圓點 | 該區間有用量 |
| 橘色 ✕ | 未知區間 —— 消耗確實發生，但取樣中斷，無法歸屬到特定區間，只能畫在附近 |
| **貼著 0 的小點** | **有取樣，但用量無變化** |
| 線斷開、空白 | **該區間沒有取樣** |

**一格可能含不只一個額度窗，百分比會超過 100%。** 曆週從週一起算，額度窗卻是
以首次使用為錨點、還可能被提前重置（Codex 實測 5 個週窗有 3 個提前重置）。
一格含兩個窗時，圖上的百分比是各窗加總 —— 那個數字是對的，hover 明細會寫出
「此週含 N 個額度窗」。單一窗的用量請讀 `v_window_summary.used_percent`。

**Codex 的 `used_percent` 只有整數解析度（1%）。** 小時粒度下多數點會貼著 0，
然後某個跳 1 —— 那不是壞掉，是資料源的性質。日／週粒度才有意義。
...
> **「涵蓋率」不是「命中率」。** prompt caching 沒有 hit／miss 的二元結果，每次請求
> 都是部分命中（實測只有 0.2% 的請求完全沒讀到快取）。它問的是「這次有多少比例
> 不用重算」。
