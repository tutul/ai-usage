...
OpenAI 於 2026-08-07~13 間多次對**全體付費用戶**重置 Codex 額度
（8/8、8/9、8/11、8/13），由 Codex 工程主管於 X 宣布，慶祝突破 1500 萬活躍用戶。
另有可存放的 reset credit（30 天到期），使用者可自行兌換。

這類重置在資料上與正常窗到期**完全相同**：百分比歸零、`resets_at` 換值，
模型已能正確處理（判為 `reset`，delta = 新百分比）。

但被切斷的「週」比正常短，不標記事後會分不清「那週沒用」與「額度被提前清掉」。
`v_window_summary.ended_early` 以「下一個窗在本窗排定的 `resets_at` 之前
就開始（容差 1 小時）」判定，並提供 `observed_duration_seconds` 供對照。

⚠️ 因此 **Codex 的「週用量」跨窗比較時要小心** —— 窗長度未必一致。
Claude 目前未觀察到此類外部重置。
