...
### 1. ~~Claude `clientID` 不應寫死~~ ✅ 已完成（2026-09-15，D-018）

> 採「使用者可填」：選單「進階」覆寫，續期時才讀。沒做自動萃取 —— 執行檔裡有**兩組**
> `CLIENT_ID`，只能靠網址字面判斷哪組是正式環境，官方改打包方式就會失效。
> 查法寫在 README。以下為當時的分析。
...
### 2. ~~`v_health.failures_total` 改為近期視窗~~ ✅ 已完成（2026-09-15，D-017）

> 新增 `failures_24h`，且兩者都不計入睡眠中斷（`slept`）。以下為當時的描述。
...
### 3. ~~UI 顯示 `ended_early`~~ ✅ 已完成（2026-09-16）

> 圖表 hover 明細在「該格含兩個以上額度窗」或「該格的窗被提前重置」時多一行說明，
> 並點出百分比是各窗加總、可能超過 100%。只在這兩種情況出現 ——
> 每一格都掛一行等於沒說。以下為當時的描述。
...
### 6. Codex 的快取 TTL（低）
已接上 Codex 的快取資料，但**「閒置後重寫」那一欄對它一律留白** —— 它的 session
紀錄沒有 TTL 分解欄位，我們也沒有辦法從外部觀測快取存活多久。套用 Claude 的
一小時門檻只是在編一個看起來合理、實際沒有根據的數字。

要解開需要能觀測到 TTL：例如刻意間隔不同長度發請求、看 `cache_write_input_tokens`
何時跳大。在有這種觀測之前，那一欄維持空白。

順帶：Codex 的 session 檔裡有 `payload.rate_limits` —— **官方用量數字、不需要憑證**。
這是 #7 在找的 Codex 備援路徑。

### 8. 用量端點 401 時是否該主動續期（低，等資料）
2026-09-13 22:28 起 Claude 連續三次 auth 失敗，15 分鐘後自行恢復，原因已查不到。
訊息來自**用量端點**的 401，而自癒只在**續期**失敗時觸發 —— 若 access token 在到期前
就被作廢，app 會一直拿它去打，直到快到期觸發續期才恢復。**這是假說，沒有證據。**

`credential_event` 的 `rejected` 事件會記下被拒當下 token 還剩多久。
下次發生時先看資料：剩很久還被拒 → 值得改成 401 時主動續期；已過期 → 是別的問題。

### 9. 請求途中沒睡著、但仍在睡眠期間的逾時（低）
睡眠中斷的判定靠「請求期間牆上時間比清醒時間多」。對照 `pmset -g log`
（09-09～14）的 116 筆網路失敗，預期約 108 筆會被判為 `slept`、1 筆是清醒時真的斷網；
其餘 7 筆請求途中沒有睡眠紀錄（4 筆在短暫喚醒期間、3 筆持續超過逾時上限但原因不明），
仍會記為 `network`。

要抓到短暫喚醒中的那 4 筆，得判斷「目前是否為短暫喚醒」—— SDK 沒有公開 API；
「`didWakeNotification` 只在完整喚醒時發出」可能是個訊號，**尚未驗證**。
為了這幾筆去依賴未驗證的機制不值得。
